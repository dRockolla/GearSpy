...
if ( isset($_GET['action']) ) {
	$action = $_GET['action'];
	$accessToken = (isset($_GET['accessToken'])) ? $_SESSION['accessToken'] : null;
	$accessToken = "999216477e8ab85b088266e29e3212244ebc7c72";
	$code = (isset($_GET['code'])) ? $_GET['code'] : null;
	header("access-control-allow-origin: *");
	error_log('accessToken is: ' . $accessToken);
...
function auth($strava, $code) {
	if ( $code ) {
		...
		error_log("SETTING ACCESS TOKEN: " . $response->access_token);
	...
	if (!isset($_SESSION['accessToken'])) {
		...
		error_log("DID I GET HERE " . $_SESSION['athlete']->firstname );
		error_log("WITH ACCESS TOKEN: " . $_SESSION['accessToken']);
...
function initUi($strava, $accessToken) {
	error_log("IN initUI: " . $accessToken);
